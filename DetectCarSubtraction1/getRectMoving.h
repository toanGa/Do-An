...
void drawListBlackRect(Size imageSize, vector<Rect> rects);
static void drawResult(Size imageSize, vector<Blob> blobs);
...
int main(void)
{
	namedWindow("drawResultBlack", CV_WINDOW_NORMAL);
	...
	while (capVideo.isOpened() && chCheckForEscKey != 27)
	{
		...
		drawListRectToMat(imgFrame2Copy, listRectCar);
		//drawListBlackRect(imgFrame2Copy.size(), listRectCar);
		cv::imshow("imgFrame2Copy", imgFrame2Copy);
		///videoExporter.write(imgFrame2Copy);
		//cv::waitKey(0);                 // uncomment this line to go frame by frame for debugging

		// now we prepare for the next iteration
		drawResult(imgFrame2Copy.size(), currentFrameBlobs);
...
void drawListBlackRect(Size imageSize, vector<Rect> rects)
{
	...
	for (int i = 0; i < size; ++i)
	{
		rectangle(image, rects.at(i), cv::Scalar(255, 255, 0), 1, 8, 0);
...
static void drawResult(Size imageSize, vector<Blob> blobs)
{
	...
	for (int i = 0; i < size; ++i)
	{
		rectangle(image, blobs.at(i).currentBoundingRect, cv::Scalar(255, 255, 255), CV_FILLED, 8, 0);
		imshow("drawResultBlack", image);
		//waitKey(-1);
	}
