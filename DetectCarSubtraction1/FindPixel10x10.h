...
int main(void)
{
	...
	while (capVideo.isOpened() && chCheckForEscKey != 27)
	{
		...
		cout << "width:" << imgFrame1Copy.cols << ";" << "height" << imgFrame1Copy.rows << endl;
		cout << "type of color is:" << imgFrame1Copy.type() << endl;
...
// test read file
		ifstream inStream("myFile/20170301-073531_0.txt");
		string test;
		while (getline(inStream, test))
		{
			cout << test << endl;
		}

// start write file
		Size_detect sizeDetect;
		sizeDetect.size_x = 100;
		sizeDetect.size_y = 100;
		Point pointStart(200, 300);
		Size size = imgFrameCvt.size();
		DetectGray detectGray(imgFrameCvt, sizeDetect, pointStart);
		char namefile[50] = "";
		strcpy(namefile, "myFile/20170301-073531_");
		string fileWrite(namefile);
		fileWrite += to_string(numFile);
		fileWrite += ".txt";

		detectGray.genarateFile(fileWrite);
		numFile++;
// stop write file
		cout << "type of gray is:" << imgFrameCvt.type() << endl;
		GaussianBlur(imgFrameCvt, imgFrameCvt, Size(9, 9), 2);
		double t1 = 10, t2 = 100;
		Canny(imgFrameCvt, imgFrameCvt, t1, t2, 3, false);
		imshow("rootImage1", imgFrameCvt);
		cout << "type of binary is:" << imgFrameCvt.type() << endl;
		//////////////////////////////////////////////////////////////////////////////////
		int x = 1;
